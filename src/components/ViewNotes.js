...
function ViewNotes() {
  ...
  const getData = async () => {
    ...
    let userType = JSON.parse(localStorage.getItem('employee'))

    if (userType) {
      setuserData(userType)
    }
    else {
      userType = JSON.parse(localStorage.getItem("admin"));
      setuserData(userType)      
    }
  ...
  return (
    <div className='viewPage d-flex flex-column justify-content-center align-items-center' style={{ padding: "7vw 2vw", paddingLeft: '23vw', width: '100%' }}>
